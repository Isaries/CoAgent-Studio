...
export const agentTypesService = {
   ...
   */
  delete: (typeName: string) => api.delete(`${API_ENDPOINTS.AGENT_TYPES.BASE}/${typeName}`),
   ...
   */
  listExternalAgents: () => api.get(`${API_ENDPOINTS.AGENTS.BASE}/external/list`),
   ...
   */
  checkA2AHealth: () => api.get<{ status: string; timestamp: string }>(API_ENDPOINTS.A2A.HEALTH)
}
