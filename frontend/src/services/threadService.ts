...
export const threadService = {
  ...
  getMessages: (threadId: string) =>
    api
      .get<ThreadMessage[]>(`${API_ENDPOINTS.THREADS.BASE}/${threadId}/messages`)
      .then((res) => res.data),

  sendMessage: (threadId: string, request: ChatRequest) =>
    api
      .post<ThreadMessage>(`${API_ENDPOINTS.THREADS.BASE}/${threadId}/messages`, request)
      .then((res) => res.data),

  // Stateless test directly bound to agent config
  testStateless: (agentId: string, request: ChatRequest) =>
    api
      .post<ChatResponse>(API_ENDPOINTS.THREADS.STATELESS(agentId), request)
      .then((res) => res.data)
}
