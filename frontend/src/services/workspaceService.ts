...
export const workspaceService = {
  ...
  createOrganization: (data: Partial<Organization>) =>
    api.post<Organization>(API_ENDPOINTS.ORGANIZATIONS.BASE, data),
  getOrganization: (orgId: string) =>
    api.get<Organization>(`${API_ENDPOINTS.ORGANIZATIONS.BASE}/${orgId}`),

  // Projects
  getProjects: (orgId?: string) =>
    api.get<Project[]>(API_ENDPOINTS.PROJECTS.BASE, { params: { org_id: orgId } }),
