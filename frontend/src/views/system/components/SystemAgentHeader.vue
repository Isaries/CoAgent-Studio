...
<template>
  <div
    class="h-14 bg-[#121212]/90 border-b border-white/5 flex items-center justify-between px-6 z-20 flex-shrink-0"
  >
    <!-- Left: Identity -->
    <div class="flex items-center gap-4">
      ...
      <label class="label cursor-pointer gap-3 group">
        <span
          class="label-text text-[10px] font-bold uppercase tracking-widest transition-colors duration-300 group-hover:text-primary"
          :class="
            sandbox.enabled
              ? 'text-primary drop-shadow-[0_0_8px_rgba(var(--p),0.6)]'
              : 'text-gray-500'
          "
          >Sandbox</span
        >
        <input
          type="checkbox"
          class="toggle toggle-primary toggle-sm border-white/10 bg-base-300"
          v-model="sandboxModel.enabled"
        />
      </label>
    </div>
    ...
    <div class="flex items-center gap-3">
      ...
      <div v-if="!sandbox.enabled" class="flex items-center gap-2 animate-fade-in transition-all">
        <div
          class="flex items-center bg-white/5 rounded-full px-3 py-1 border border-white/5 focus-within:border-primary/50 transition-colors"
        >
          ...
          <button
            @click="$emit('saveKey')"
            class="ml-2 text-primary hover:text-primary-focus transition-colors"
        ...
        <button
          v-if="maskedApiKey"
          @click="$emit('clearKey')"
          class="btn btn-circle btn-xs btn-ghost text-error opacity-50 hover:opacity-100"
        >
