...
const deleteAgent = async (agent: AgentConfig) => {
  if (!(await confirmDialog('Delete Agent', `Are you sure you want to delete "${agent.name}"?`)))
    return
  try {
    ...
    <div v-else-if="agents.length === 0" class="card bg-base-100 shadow-xl">
      <div class="card-body items-center text-center py-12">
        ...
        <p class="text-base-content/60 mb-4">
          Create your first global agent template to get started.
        </p>
      ...
      >
        <div class="card-body">
          ...
          <div class="card-actions justify-end mt-4">
            ...
            <button class="btn btn-error btn-sm btn-outline" @click="deleteAgent(agent)">
              Delete
            </button>
          </div>
        </div>
