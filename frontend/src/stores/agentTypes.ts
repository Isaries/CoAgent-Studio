...
export const useAgentTypesStore = defineStore('agentTypes', () => {
  ...
  const instructorTypes = computed(() =>
    types.value.filter(
      (t) => t.category === AgentCategory.INSTRUCTOR || t.category === 'instructor'
    )
  )

  const participantTypes = computed(() =>
    types.value.filter(
      (t) => t.category === AgentCategory.PARTICIPANT || t.category === 'participant'
    )
  )
  ...
  const customTypes = computed(() => types.value.filter((t) => !t.is_system))

  const getTypeByName = (typeName: string) => types.value.find((t) => t.type_name === typeName)
