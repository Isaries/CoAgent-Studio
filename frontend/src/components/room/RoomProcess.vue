...
async function deleteProcess(processId: string) {
  ...
  try {
    const success = await workspaceStore.deleteTask(processId)
    if (success) {
...
<template>
  <div class="flex-1 overflow-hidden bg-base-200/50 p-4 flex gap-4 relative">
    ...
    <div class="w-72 bg-base-100 rounded-box shadow-sm flex flex-col border border-base-200">
      <div
        class="p-4 border-b border-base-200 flex justify-between items-center bg-base-50 rounded-t-box"
      >
      ...
      <div class="flex-1 overflow-y-auto p-2 space-y-1">
        ...
        >
          <div class="flex items-start gap-3">
            <div
              class="mt-1 p-1.5 rounded bg-base-200 group-hover:bg-primary/10 text-primary-content group-hover:text-primary transition-colors"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="16"
            ...
            <div class="flex-1 min-w-0">
              <div
                ...
                :class="{ 'text-primary': selectedProcessId === proc.id }"
              >
                {{ proc.title }}
              </div>
              <div class="text-xs text-base-content/50 mt-0.5 flex items-center gap-1">
                <span
                  v-if="isAgent(proc.last_modified_by, authStore.user?.id)"
                  class="text-primary font-xs flex items-center gap-0.5"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="10"
            ...
            <button
              ...
              @click.stop="deleteProcess(proc.id)"
              title="Delete"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
        ...
        <div
          v-if="workspaceStore.processes.length === 0"
    ...
    <!-- Process Viewer -->
    <div
      class="flex-1 bg-base-100 rounded-box shadow-sm border border-base-200 overflow-hidden relative"
    >
      <ProcessViewer
