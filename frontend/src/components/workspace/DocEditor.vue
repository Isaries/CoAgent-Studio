...
<template>
  <div
    class="flex flex-col h-full bg-base-100 rounded-box shadow-sm overflow-hidden border border-base-200"
  >
    <!-- Toolbar -->
    <div
      v-if="editor"
      class="flex items-center gap-1 p-2 border-b border-base-200 bg-base-200/30 overflow-x-auto"
    >
      <button
      ...
      >
        ...
        >
          <path
            d="M17.3 19c.266-1.12.23-2.61-.4-4.1-.73-1.74-2.5-1.9-4.9-1.9H6v4h6c1.1 0 1.967.1 2.567.3.6.2 1.033.7 1.3 1.5.067.2.133.433.2.7"
          ></path>
          <path d="M14 5c.667.667 1 1.667 1 3 0 2-1 3.5-3 3.5H6V5h8z"></path>
    ...
    <editor-content
      :editor="editor"
      class="flex-1 overflow-y-auto p-4 prose prose-sm max-w-none focus:outline-none"
    />

    <!-- Status Bar -->
    <div
      class="flex items-center justify-between px-4 py-2 text-xs text-base-content/50 border-t border-base-200"
    >
      <span class="flex gap-2">
        {{ editor?.getCharacterCount?.() ?? editor?.getText()?.length ?? 0 }} characters
        <span
          v-if="remoteChangePending"
          class="text-warning font-bold cursor-pointer hover:underline"
          @click="applyRemoteContent"
        >
