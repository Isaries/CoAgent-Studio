...
<template>
  <div class="h-full flex flex-col bg-base-100 border-l border-base-300 w-full sm:w-80">
    <div class="p-4 border-b border-base-200 flex justify-between items-center bg-base-200/50">
      ...
      <button v-if="$attrs.onClose" @click="$emit('close')" class="btn btn-ghost btn-xs btn-circle">
        ✕
      </button>
    </div>
        ...
        >
          <div class="card-body p-3">
            <div class="flex justify-between items-start">
              ...
              <div class="badge badge-ghost badge-xs font-mono text-[10px]">
                {{ ver.model_provider }}
              </div>
            </div>

            <div
              class="mt-2 text-xs truncate text-base-content/60 font-mono bg-base-200/50 p-1 rounded"
            >
