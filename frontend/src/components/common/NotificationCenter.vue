...
<template>
  <div class="dropdown dropdown-end">
    ...
    <div
      v-if="isOpen"
      tabindex="0"
      class="dropdown-content z-50 menu p-0 shadow-lg bg-base-100 rounded-box w-80 border border-base-300"
    >
          ...
          <div class="flex items-start gap-2">
            ...
            <div class="flex-1 min-w-0">
              <div class="flex items-center justify-between">
                <span class="text-sm font-medium truncate" :class="typeClass(item.type)">{{
                  item.title
                }}</span>
      ...
      <div v-if="items.length > 0" class="border-t border-base-300 px-4 py-2">
        <button class="btn btn-ghost btn-xs btn-block" @click="store.clear()">
