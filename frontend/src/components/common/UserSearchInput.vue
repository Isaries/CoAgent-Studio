...
<template>
  <div class="form-control w-full relative">
    ...
    <ul
      ...
      class="absolute top-full left-0 w-full bg-base-100 shadow-xl rounded-box z-50 p-2 mt-1 border border-base-200 max-h-60 overflow-y-auto"
    >
      <li v-for="user in searchResults" :key="user.id">
        <a @click="selectUser(user)" class="block p-2 hover:bg-base-200 rounded cursor-pointer">
          ...
          <div class="text-xs opacity-70">
            {{ user.email }} <span v-if="user.username">({{ user.username }})</span>
          </div>
        </a>
      </li>
    </ul>
    ...
    <div
      ...
      class="absolute top-full left-0 w-full bg-base-100 p-2 mt-1 text-sm text-center opacity-50 z-50"
    >
      No users found.
    </div>
    ...
    <div
      v-if="searchLoading"
      class="absolute right-3 top-[3rem] loading loading-spinner loading-xs text-secondary"
    ></div>
  </div>
</template>
