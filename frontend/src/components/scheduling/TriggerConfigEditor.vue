...
const { triggerConfig, toggleCondition, isConditionEnabled, setLogic, loadConfig } =
...
const conditionLabels: Record<string, { label: string; unit: string; desc: string }> = {
  [TriggerCondition.MESSAGE_COUNT]: {
    label: 'Message Count',
    unit: 'messages',
    desc: 'Trigger after N user messages'
  },
  [TriggerCondition.TIME_INTERVAL_MINS]: {
    label: 'Time Interval',
    unit: 'mins',
    desc: 'Trigger every N minutes'
  },
  [TriggerCondition.USER_SILENT_MINS]: {
    label: 'User Silence',
    unit: 'mins',
    desc: 'Trigger after N minutes of user silence'
  }
}
    ...
    <div class="card-body gap-5">
      ...
      <div class="space-y-3">
        ...
        <p class="text-xs opacity-50">
          Select which conditions to evaluate.
          {{
            triggerConfig.logic === 'or'
              ? 'Agent speaks when ANY condition is met.'
              : 'Agent speaks when ALL conditions are met.'
          }}
        </p>

        <!-- Condition Checkboxes -->
        <div
          v-for="(info, key) in conditionLabels"
          :key="key"
          class="border rounded-lg p-3 bg-base-50"
        >
          ...
          <div
            v-if="isConditionEnabled(key as TriggerCondition)"
            class="mt-2 ml-8 flex items-center gap-2"
          >
            <input
              type="number"
      ...
      <div class="flex items-center gap-3">
        <select
          v-model="triggerConfig.trigger.context_strategy.type"
      ...
      <div class="space-y-2">
        <select
          v-model="triggerConfig.close.strategy"
          class="select select-bordered select-sm w-full"
        >
          <option :value="CloseStrategy.NONE">None — agent never sleeps</option>
          <option :value="CloseStrategy.AGENT_MONOLOGUE">
            Agent Monologue — sleep after N consecutive replies
          </option>
          <option :value="CloseStrategy.USER_TIMEOUT">
            User Timeout — sleep if user doesn't reply in N mins
          </option>
        </select>

        <div
          v-if="triggerConfig.close.strategy === CloseStrategy.AGENT_MONOLOGUE"
          class="flex items-center gap-2 ml-2"
        >
          <span class="text-xs">Max consecutive replies:</span>
          <input
            type="number"
            v-model.number="triggerConfig.close.monologue_limit"
        ...
        <div
          v-if="triggerConfig.close.strategy === CloseStrategy.USER_TIMEOUT"
          class="flex items-center gap-2 ml-2"
        >
          <span class="text-xs">Timeout:</span>
          <input
            type="number"
            v-model.number="triggerConfig.close.timeout_mins"
            class="input input-bordered input-xs w-16"
            :min="1"
          />
      ...
      <div class="space-y-2">
        <div class="flex items-center gap-2">
          ...
          <select
            v-model.number="triggerConfig.self_modification.duration_hours"
        ...
        <p class="text-xs opacity-40 ml-2">
          {{
            triggerConfig.self_modification.duration_hours === 0
              ? 'Agent cannot modify its own triggers.'
              : triggerConfig.self_modification.duration_hours === -1
                ? 'Agent modifications are permanent.'
        ...
        <div v-if="triggerConfig.self_modification.duration_hours !== 0" class="ml-2 space-y-1">
          ...
          <div v-for="(info, key) in conditionLabels" :key="key" class="flex items-center gap-2">
            ...
            <input
              ...
              @input="
                if (!triggerConfig.self_modification.bounds)
                  triggerConfig.self_modification.bounds = {}
                if (!triggerConfig.self_modification.bounds[key])
                  triggerConfig.self_modification.bounds[key] = {}
                triggerConfig.self_modification.bounds[key].min = Number(
            ...
            <input
              ...
              @input="
                ...
                if (!triggerConfig.self_modification.bounds[key])
                  triggerConfig.self_modification.bounds[key] = {}
                triggerConfig.self_modification.bounds[key].max = Number(
                  ($event.target as HTMLInputElement).value
                )
              "
              class="input input-bordered input-xs w-16"
      ...
      <div class="space-y-2">
        <label class="flex items-center gap-2 cursor-pointer">
          <input
            ...
            v-model="triggerConfig.state_reset.enabled"
          />
        ...
        <div v-if="triggerConfig.state_reset.enabled" class="ml-8 flex items-center gap-2">
          ...
          <input
            type="number"
            v-model.number="triggerConfig.state_reset.interval_days"
          ...
          <input
            type="time"
            v-model="triggerConfig.state_reset.reset_time"
