    ...
    async with async_session() as session:
        ...
        for room in rooms:
            ...
            stmt = (
                ...
                .where(RoomAgentLink.is_active is True)
            )
                ...
                nodes.append(
                    {
                        "id": str(student.id),
                        "type": "agent",
                        "config": {"agent_id": str(student.id)},
                        "position": {"x": 250, "y": 200},
                    }
                )
                edges.append(
                    {
                        "id": f"edge-start-{student.id}",
                        "source": "start",
            ...
            if teacher:
                nodes.append(
                    {
                        "id": str(teacher.id),
                        "type": "agent",
                        "config": {"agent_id": str(teacher.id)},
                ...
                if student:
                    edges.append(
                        {
                            "id": f"edge-{student.id}-{teacher.id}",
                            "source": str(student.id),
                            "target": str(teacher.id),
                            "type": "forward",
                        }
                    )
            ...
            if teacher and student:
                ...
                nodes.append(
                    {
                        "id": router_id,
                        "type": "router",
                        "config": {"condition": "is_approved"},
                        "position": {"x": 750, "y": 200},
                    }
                )
                edges.append(
                    {
                        "id": f"edge-{teacher.id}-{router_id}",
                        "source": str(teacher.id),
                        "target": router_id,
                        "type": "forward",
                    }
                )
                ...
                edges.append(
                    {
                        "id": f"edge-{router_id}-{student.id}",
                        "source": router_id,
                        "target": str(student.id),
                        "type": "evaluate",
                        "condition": "rejected",
            ...
            workflow = RoomWorkflow(
                id=uuid.uuid4(), room_id=room.id, graph_data=graph_data, is_active=True
            )
