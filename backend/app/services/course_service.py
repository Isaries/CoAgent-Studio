...
from app.models.course import Course, CourseCreate, CourseUpdate, UserCourseLink, CourseMemberUpdate
...
class CourseService:
    ...
    async def update_course(self, course_id: UUID, course_in: CourseUpdate, current_user: User) -> Course:
    ...
    async def delete_course(self, course_id: UUID, current_user: User) -> Course:
        try:
            ...
            await self.session.exec(delete(UserCourseLink).where(UserCourseLink.course_id == course_id))
            ...
            try:
                await self.session.exec(delete(AnalyticsReport).where(AnalyticsReport.course_id == course_id))
            except Exception as e:
                print(f"WARNING: Failed to delete AnalyticsReport. Table might be missing or DB error. Error: {e}")
                # We allow proceeding because if table is missing, data is effectively 'gone' or unreachable anyway.
            
            # 4. Rooms & Messages
            print("DEBUG: Fetching Rooms...")
            rooms_result = await self.session.exec(select(Room.id).where(Room.course_id == course_id))
            ...
            if room_ids:
                ...
                await self.session.exec(delete(UserRoomLink).where(UserRoomLink.room_id.in_(room_ids)))
            ...
            raise HTTPException(status_code=500, detail=f"Delete Failed: {str(e)} Type: {type(e).__name__}")

    async def enroll_user(self, course_id: UUID, email: Optional[str], user_id: Optional[UUID], role: str, current_user: User) -> str:
        course = await self.get_course_by_id(course_id)
        if not course:
            raise HTTPException(status_code=404, detail="Course not found")
            
        # Permission: Admin, Owner, or TA
        is_admin_owner = current_user.role in [UserRole.ADMIN, UserRole.SUPER_ADMIN] or course.owner_id == current_user.id
        ...
        if user_id:
            ...
            user_to_enroll = (await self.session.exec(select(User).where(User.email == email))).first()
    ...
    async def update_member_role(self, course_id: UUID, user_id: UUID, role: str, current_user: User):
        course = await self.get_course_by_id(course_id)
        if not course:
            raise HTTPException(status_code=404, detail="Course not found")
