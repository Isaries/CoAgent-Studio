...
class EntityNode(BaseModel):
    ...
    name: str = Field(
        description="Canonical name of the entity (e.g. 'Alice', 'PostgreSQL', 'JWT Token')"
    )
    type: str = Field(
        description="Entity type: PERSON, AGENT, CONCEPT, TECHNOLOGY, ARTIFACT, ISSUE"
    )
...
class EntityRelationship(BaseModel):
    ...
    relation: str = Field(
        description="Relationship type: DISCUSSES, CREATES, RESOLVES, STRUGGLES_WITH, MENTIONS, COLLABORATES_WITH, DEPENDS_ON"
    )
...
class CommunityReport(BaseModel):
    ...
    key_entities: List[str] = Field(
        default_factory=list, description="Most important entity names in this community"
    )
...
class QueryIntent(BaseModel):
    ...
    entities: List[str] = Field(
        ...
        description="Entity names mentioned in the question (for local search)",
    )
...
class GraphQueryResponse(BaseModel):
    ...
    sources: List[str] = Field(
        default_factory=list, description="Key entities or communities referenced"
    )
