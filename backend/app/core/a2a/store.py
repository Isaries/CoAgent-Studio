...
class A2AMessageRecord(SQLModel, table=True):
    ...
    def from_a2a_message(cls, msg: A2AMessage) -> "A2AMessageRecord":
        ...
        try:
            content_str = (
                json.dumps(msg.content) if not isinstance(msg.content, str) else msg.content
            )
...
class A2AMessageStore:
    ...
    async def get_by_message_id(self, message_id: UUID) -> Optional[A2AMessageRecord]:
        ...
        stmt = select(A2AMessageRecord).where(A2AMessageRecord.message_id == str(message_id))
    ...
    async def get_by_room(
        ...
        """
        stmt = select(A2AMessageRecord).where(A2AMessageRecord.room_id == room_id)
        if message_type:
