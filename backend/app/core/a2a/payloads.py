...
class EvaluationRequestPayload(BaseModel):
    ...
    proposal: str = Field(
        ..., min_length=1, max_length=50000, description="The proposed message content"
    )
...
class BroadcastPayload(BaseModel):
    ...
    format: Literal["text", "markdown", "html"] = Field(
        default="text", description="Content format"
    )
...
class SystemPayload(BaseModel):
    ...
    event: Literal["join", "leave", "error", "info", "warning"] = Field(
