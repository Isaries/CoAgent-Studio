...
from app.models.artifact import ArtifactCreate, ArtifactRead, ArtifactType, ArtifactUpdate
...
@router.post(
    "/{room_id}/artifacts", response_model=ArtifactRead, status_code=status.HTTP_201_CREATED
)
async def create_artifact(
    ...
    if data.type not in valid_types:
        raise HTTPException(
            status_code=400, detail=f"Invalid artifact type. Must be one of: {valid_types}"
        )
...
async def delete_artifact(
    ...
    success = await service.delete_artifact(
        artifact_id, deleted_by=current_user.id, hard_delete=hard_delete
    )
