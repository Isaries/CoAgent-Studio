...
async def test_agent_append_concurrency_lost_update(db_session: AsyncSession):
    ...
    doc_content_v1 = {
        "type": "doc",
        "content": [{"type": "paragraph", "content": [{"type": "text", "text": "Original V1"}]}],
    }
    ...
    artifact_v2 = await service.update_artifact(
        artifact_v1.id, ArtifactUpdate(content=doc_content_v2), modified_by=mock_user_id
    )
    ...
    with patch(
        "app.services.artifact_service.ArtifactService.get_artifact", new_callable=AsyncMock
    ) as mock_get:
