...
class TestArtifactService:
    ...
    async def test_list_artifacts_by_type(self, db_session, mock_superuser: User):
        ...
        await service.create_artifact(
            room_id, ArtifactCreate(type="task", title="Task 1", content={}), mock_superuser.id
        )
        await service.create_artifact(
            room_id, ArtifactCreate(type="doc", title="Doc 1", content={}), mock_superuser.id
        )
        await service.create_artifact(
            room_id, ArtifactCreate(type="task", title="Task 2", content={}), mock_superuser.id
        )
    ...
    @pytest.mark.asyncio()
    async def test_update_artifact_version_increment(
        self, db_session, mock_superuser: User, mock_artifact_data
    ):
        ...
        updated = await service.update_artifact(
            artifact.id, ArtifactUpdate(title="Updated Title"), modified_by=mock_superuser.id
        )
    ...
    @pytest.mark.asyncio()
    async def test_update_artifact_optimistic_locking_conflict(
        self, db_session, mock_superuser: User, mock_artifact_data
    ):
        ...
        await service.update_artifact(
            artifact.id, ArtifactUpdate(title="First Update"), modified_by=mock_superuser.id
        )
