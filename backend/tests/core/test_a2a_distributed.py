...
class MockRedis:
        ...
        for stream_name, start_id in streams_dict.items():
            if stream_name in self.streams:
                if start_id == ">":  # New messages only
                    entries = (
                        self.streams[stream_name][-count:] if count else self.streams[stream_name]
                    )
                    if entries:
    ...
    async def xdel(self, stream, *entry_ids):
        ...
        if stream in self.streams:
            self.streams[stream] = [
                (eid, data) for eid, data in self.streams[stream] if eid not in entry_ids
            ]
...
class TestDeadLetterQueue:
    ...
    async def test_message_moved_to_dlq_after_max_retries(self):
        ...
        with patch("app.core.a2a.distributed.redis") as mock_redis_module:
            ...
            await dispatcher._process_entry("teacher", stream_name, entry_id, data, failing_handler)
