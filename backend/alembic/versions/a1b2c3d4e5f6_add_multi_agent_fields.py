...
def upgrade() -> None:
    ...
    op.add_column(
        "agentconfig",
        sa.Column("external_config", postgresql.JSONB(astext_type=sa.Text()), nullable=True),
    )
    ...
    op.create_index(
        op.f("ix_agenttypemetadata_type_name"), "agenttypemetadata", ["type_name"], unique=True
    )
    op.create_index(
        op.f("ix_agenttypemetadata_category"), "agenttypemetadata", ["category"], unique=False
    )
