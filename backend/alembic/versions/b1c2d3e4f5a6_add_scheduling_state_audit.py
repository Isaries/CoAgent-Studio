...
def upgrade() -> None:
    # --- 1. user_api_key ---
    op.add_column(
        "user_api_key", sa.Column("is_active", sa.Boolean(), server_default="true", nullable=False)
    )
    op.add_column("user_api_key", sa.Column("schedule_config", JSONB, nullable=True))

    # --- 2. roomagentlink ---
    op.add_column(
        "roomagentlink", sa.Column("is_active", sa.Boolean(), server_default="true", nullable=False)
    )
    ...
    op.add_column(
        "roomagentlink",
        sa.Column("updated_by", UUID(as_uuid=True), sa.ForeignKey("user.id"), nullable=True),
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "room_id", UUID(as_uuid=True), sa.ForeignKey("room.id"), nullable=False, index=True
        ),
        sa.Column(
            "agent_id",
            UUID(as_uuid=True),
        ...
        sa.Column(
            "message_count_since_last_reply", sa.Integer(), server_default="0", nullable=False
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "created_at", sa.DateTime(), server_default=sa.func.now(), nullable=True, index=True
        ),
    )
